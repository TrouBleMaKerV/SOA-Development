
package com.soa12.assignment10.schemaModel;

import javax.xml.ws.WebFault;


/**
 * This class was generated by the JAX-WS RI.
 * JAX-WS RI 2.2.9-b14002
 * Generated source version: 2.2
 * 
 */
@WebFault(name = "AuthFault")
//@WebFault(name = "AuthFault", targetNamespace = "http://jw.nju.edu.cn/schema")
public class PasswordErrorException
    extends Exception
{

    /**
     * Java type that goes as soapenv:Fault detail element.
     * 
     */
    private AuthFaultType faultInfo;

    public PasswordErrorException(String reason, String id, String message) {
        super(message);
        faultInfo.setReason(reason);
        faultInfo.setId(id);
    }

    /**
     * 
     * @param faultInfo
     * @param message
     */
    public PasswordErrorException(String message, AuthFaultType faultInfo) {
        super(message);
        this.faultInfo = faultInfo;
    }

    /**
     * 
     * @param faultInfo
     * @param cause
     * @param message
     */
    public PasswordErrorException(String message, AuthFaultType faultInfo, Throwable cause) {
        super(message, cause);
        this.faultInfo = faultInfo;
    }

    /**
     * 
     * @return
     *     returns fault bean: cn.edu.nju.jw.schema.AuthFaultType
     */
    public AuthFaultType getFaultInfo() {
        return faultInfo;
    }

}
